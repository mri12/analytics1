...
fit1
summary(fit1)
# when F-statistic => p value is less than 0.05 - then the model exists
...
ndata1 = data.frame(X=c(1.5,2,3,4,5))
predict(fit1,newdata=ndata1,type='response')
...
#Display 4 plots one by one
plot(fit1)

#Display 4 plots in single frame
par(mfrow=c(2,2))
plot(fit1)
